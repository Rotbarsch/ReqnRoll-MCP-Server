...
This is a Proof-of-Concept (PoC) implementation of a MCP (Model Context Protocol) server for [Reqnroll](https://github.com/reqnroll/Reqnroll) bindings, to enable AI agents to get info on the available bindings.
...
1. If not open already, open the GitHub Copilot chat, eg by pressing `CTRL+ALT+I` or whatever shortcut is configured for your environment.
2. Make sure your GitHub Copilot is set to "Agent" on the bottom left of the chat window.
3. Click the small icon depicting a wrench and screwdriver on the bottom of the chat, next to selection of the model. At the top of the Visual Studio Code window, a list of available MCP servers opens.
4. Click the small icon labeled "Add MCP Server..." on the top of the list of available MCP servers. 
...
7. Enter a unique and informative name for your configuration, eg "ReqnRoll MCP Server".
...
```
9. Save the file. Notice the small `Start` prompt on top of the JSON node describing your newly added MCP server. Click it to start the server. If anything goes wrong, Visual Studio Code will display the console output of the server with a detailed stracktrace.
10. Clicking the Wrench and Screwdriver icon again will now show your MCP server. If the checkbox next to it, check it.
11. The Github Copilot is now ready for use. Try prompting it with:
```
List all avilable ReqnRoll bindings.
```
...
- *Server ID*: Enter a unique and informative name for your configuration, eg "ReqnRoll MCP Server".
- *Type*: Select "stdio".
...
6. The Github Copilot is now ready for use. Try prompting it with:
```
List all avilable ReqnRoll bindings.
```
